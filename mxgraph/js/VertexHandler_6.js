    ...
    if(inspectioUtils.isEventModel(this.state.cell)) {
        ...
        try {
            if(xOffset === 0 && widthOffset !== 0) {
                ...
                this.graph.moveCells(timehandles, (widthOffset - this.lastWidthOffset) * -1, 0, false);
            } else if (xOffset !== 0) {
                var lanehandles = this.graph.model.getChildren(this.state.cell).filter(child => inspectioUtils.isLaneHandle(child));
                this.graph.moveCells(lanehandles, (xOffset - this.lastXOffset) * -1, 0, false);
            }
            this.lastXOffset = xOffset;
            this.lastWidthOffset = widthOffset;
        }
        finally {
