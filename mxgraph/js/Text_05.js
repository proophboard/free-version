...
mxText.prototype.redraw = function()
{
    ...
    else
    {
        ...
        var doRedrawText = () => {
            var untouchedValue = this.value;
            this.value = graph.tagLinks(this.value);

            mxShape.prototype.redraw.apply(this, arguments);

            const tagLinks = currentNode.querySelectorAll('.mxgraph-tag-lookup');
            tagLinks.forEach(tagLink => {
                const tagLinkClick = (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    graph.lookupElement(tagLink.getAttribute('href').toLowerCase());
                    return false;
                }
                tagLink.addEventListener('click', tagLinkClick, false);
            })

            this.value = untouchedValue;
