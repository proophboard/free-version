...
Graph = function(container, model, renderHint, stylesheet, themes)
{
	...
	this.showCellMetadata = function (focusMetadataEditor) {
		if(this.showMetadataListener) {
			this.showMetadataListener(focusMetadataEditor || false);
		}
	}
	...
	this.setCodySuggestEnabled = function (enabled) {
		...
		if(!enabled) {
			var setDashedStyle = this.editorUi.actions.get('set_dashed_style');

			if(setDashedStyle) {
				setDashedStyle.funct();
			}
		}
	}
	...
	if (typeof mxVertexHandler !== 'undefined')
	{
		...
		this.addListener(mxEvent.CELLS_MOVED, function (sender, evt) {
            if(evt.properties.cells.length > 0) {
            	...
            	console.log("Checking moved cells: ", cells);
            	let cellIndex = 0;
	...
	mxPanningHandler.prototype.mouseUp = function(sender, me)
	{
		if (this.active)
		{
			...
			if (!cellSelected && this.dx != null && this.dy != null)
			{
				...
				if(this.graph.liteMode || this.graph.effectRunning > 0 || panTime > 1000) {
					this.fireEvent(new mxEventObject(mxEvent.PAN_END, 'event', me));
					this.reset();
					this.captureMouseMove = false;
					this.physicsActive = false;
...
if (typeof mxVertexHandler != 'undefined')
{
	(function()
	{
		...
		Graph.prototype.syncOverlappingChildrenToContainer = function(container, bound, cellIgnoreList, ignoreCellsWithContainerParent) {
			...
			const childrenToCheck = allChildren.filter(child => !child.isEdge());
			...
			window.setTimeout(() => {
				...
				try {
					cells.forEach(cell => {
						...
						// Add lane handle tag to all roles so that they work out-of-the-box in an Event Modeling Slice
						if(inspectioUtils.isOfType(cell, ispConst.TYPE_ROLE)) {
							inspectioUtils.addTag(cell, ispConst.TAG_LANE_HANDLE, this);
						}
					});
			...
			try {
				...
				if(vertexIsResized && inspectioUtils.isSlice(this.state.cell)) {
					if(xOffset === 0 && widthOffset !== 0) {
						...
						this.graph.moveCells(timehandles, widthOffset * -1, 0, false);
					} else if (xOffset !== 0) {
						var lanehandles = this.graph.model.getChildren(this.state.cell).filter(child => inspectioUtils.isLaneHandle(child));
						this.graph.moveCells(lanehandles, this.lastXOffset, 0, false);
						this.graph.moveCells(lanehandles, xOffset * -1, 0, false);
					}
