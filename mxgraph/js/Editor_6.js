 ...
 */
Editor = function(chromeless, themes, model, graph, editable, eventModelingEnabled)
{
	mxEventSource.call(this);
	this.chromeless = (chromeless != null) ? chromeless : this.chromeless;
	this.initStencilRegistry();
	this.graph = graph || this.createGraph(themes, model);
	this.graph.setEventModelingEnabled(eventModelingEnabled);
...
(function()
{
	...
	mxGraphView.prototype.validateBackgroundStyles = function()
	{
		...
		if (graph.view.backgroundPageShape != null)
		{
			...
			graph.container.className = 'geDiagramContainer geDiagramBackdrop ' + graph.getUserSelectContainerClass();
			canvas.style.backgroundImage = 'none';
			canvas.style.backgroundColor = '';
		}
		else
		{
			graph.container.className = 'geDiagramContainer ' + graph.getUserSelectContainerClass();
	...
	mxGraphView.prototype.createSvgGrid = function(color)
	{
		...
		if(this.scale <= 0.4 && this.scale > 0.3) {
			tmp *= 2;
		}
		...
		var pattern = '<path d="M ' + tmp2 + ' 0 L 0 0 0 ' + tmp2 + '" fill="none" stroke="' + color + '" stroke-width="3"/>';

		if(this.graph.getGraphStylesheet()[ispConst.STYLE_GRAPH_GRID_STYLE] === ispConst.GRID_STYLE_DOTTED) {
			pattern = '<circle cx="1" cy="1" r="2" fill="'+color+'" stroke="'+color+'" />';
		}
	...
	/* Disabled, because otherwise sticky are not selected if container is already selected
	...
	{
		/* Do not automatically select parent cells, to avoid deletion of containers
