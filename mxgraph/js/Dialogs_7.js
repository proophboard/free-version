...
var ColorDialog = function(editorUi, color, apply, cancelFn)
{
	...
	jscolor.picker.box.style.width = '215px';
	jscolor.picker.box.style.height = '120px';
	jscolor.picker.box.style.paddingBottom = '10px';
	div.appendChild(jscolor.picker.box);

	var center = document.createElement('div');
	center.style.paddingLeft = '10px';
...
var LinkDialog = function(editorUi, initialValue, btnLabel, fn)
{
	...
	linkInput.style.borderBottom = '1px solid grey';
 ...
 * Constructs a new link dialog.
 */
var IconDialog = function(editorUi, fn, noStyle)
{
    var div = document.createElement('div');
    div.className = 'geFaIconsContainer';
    ...
    var iconSearchInput = document.createElement('input');
    iconSearchInput.setAttribute('placeholder', mxResources.get('searchIcons'));
    iconSearchInput.setAttribute('type', 'text');
    iconSearchInput.style.backgroundImage = 'url(\'' + Dialog.prototype.clearImage + '\')';
    iconSearchInput.style.backgroundRepeat = 'no-repeat';
    iconSearchInput.style.backgroundPosition = '100% 50%';
    ...
    mxEvent.addListener(iconSearchInput, 'focus', () => {
    	iconSearchInput.classList.add('active');
	})

	mxEvent.addListener(iconSearchInput, 'blur', () => {
		iconSearchInput.classList.remove('active');
	})

    inner.appendChild(iconSearchInput);
    inner.appendChild(cross);
    div.appendChild(inner);

    const icons = document.createElement('div');
    icons.className = 'geFaIcons';

    const coloredIcon = (svg) => svg.replace('path d', 'path fill="currentcolor" d');

    const pickIcon = (evt, svg) => {
		if(noStyle) {
			editorUi.hideDialog();
			fn(svg);
			return;
        ...
        if(icon.getElementsByTagName('svg').length > 0) {
            icon = icon.getElementsByTagName('svg')[0];
        }
        editorUi.hideDialog();
        fn(icon.parentElement.innerHTML);
	};

    const createIcon = (title, svg) => {
        const icon = document.createElement('div');
        icon.className = 'geFaIcon';
        icon.innerHTML = '<i>' + coloredIcon(svg) + '</i>';
        icon.title = title;
        icon.getElementsByTagName('svg')[0].style.width = '30px';
        icon.getElementsByTagName('svg')[0].style.height = '30px';

        mxEvent.addListener(icon, 'click', (evt) => {
			pickIcon(evt, svg);
		});
	...
	const isExpandedCategory = (cat) => {
    	return cat.getElementsByTagName('i')[0].classList.contains('minus');
	}

	const toggleIcons = (cat, iContainer) => {
    	if(isExpandedCategory(cat)) {
    		const i = cat.getElementsByTagName('i')[0];
            ...
            iContainer.style.display = 'inherit';
		}
	}
    ...
    const initIcons = () => {
        ...
        commonH3.className = 'ui header geIconCategory';
        commonH3.innerHTML = 'Common';
        const minusI = document.createElement('i');
        minusI.className = 'small minus icon';
        commonH3.prepend(minusI);

        icons.append(commonH3);

        const iconsContainer = document.createElement('div');

        mxEvent.addListener(commonH3, 'click', () => toggleIcons(commonH3, iconsContainer))

    	for (const commonI in faDefinitions.Common) {
            if(faDefinitions.Common.hasOwnProperty(commonI)) {
            	const catAlias = faDefinitions.Common[commonI];
                const icon = createIcon(faDefinitions[catAlias][commonI], faIcons[catAlias][commonI]);
                iconsContainer.append(icon);
            }
		}

		icons.append(iconsContainer);

    	for(const cat in faDefinitions) {
    		if(cat === 'Common') {
    			continue;
			}

    		if(faDefinitions.hasOwnProperty(cat)) {
                const catH3 = document.createElement('h3');
                const catIconContainer = document.createElement('div');
                catH3.className = 'ui header geIconCategory';
                catH3.innerHTML = cat;

                const plusI = document.createElement('i');
                plusI.className = 'small plus icon';
                catH3.prepend(plusI);

                icons.append(catH3);
                icons.append(catIconContainer);

                catIconContainer.style.display = 'none';

                mxEvent.addListener(catH3, 'click', () => toggleIcons(catH3, catIconContainer));

                for(const i in faDefinitions[cat]) {
                    if(faIcons[cat].hasOwnProperty(i)) {
                        const icon = createIcon(faDefinitions[cat][i], faIcons[cat][i]);
                        catIconContainer.append(icon);
                    }
                }
			}
	...
	const searchIcons = (term) => {
		...
		for(const cat in faDefinitions) {
			...
			if(faDefinitions.hasOwnProperty(cat)) {
				for(const i in faDefinitions[cat]) {
					if(faDefinitions[cat].hasOwnProperty(i)) {
						if (faDefinitions[cat][i].toLowerCase().search(term.toLowerCase()) !== -1) {
							matches.push({id: i,cat: cat, title: faDefinitions[cat][i], svg: faIcons[cat][i]})
						}
					}
				}
			}
		...
		if(matches.length === 0) {
            icons.innerHTML = mxResources.get('noIconsFound');
        } else {
    		icons.innerHTML = '';

    		let iconIndex = {};

    		matches.forEach(iconDef => {
    			if(!iconIndex.hasOwnProperty(iconDef.id)) {
                    const icon = createIcon(iconDef.title, iconDef.svg);
                    icons.append(icon);

                    iconIndex[iconDef.id] = true;
				}
    		});
		}
	};

    if(typeof faIcons === 'undefined') {
        inspectioUtils.require('mxgraph/js/faIcons.js', initIcons);
        icons.innerHTML = '<div class="ui active dimmer">\n' +
    ...
    mxEvent.addListener(iconSearchInput, 'keyup', function(e)
    {
        if (e.keyCode == 13)
        {
        	const foundIcons = icons.getElementsByClassName('geFaIcon');

        	if(foundIcons.length > 0) {
                editorUi.hideDialog();
                fn(foundIcons[0].getElementsByTagName('i')[0].innerHTML);
			}
        } else {
        	if(iconSearchInput.value !== '') {
                searchIcons(iconSearchInput.value);
            } else {
        		initIcons();
			}
		}
...
var LayersWindow = function(editorUi, x, y, w, h)
{
	...
	listDiv.style.bottom = (parseInt(tbarHeight) + 14) + 'px';
	...
	{
		if (graph.isEnabled() && selectionLayer)
		{
			if(selectionLayer.getId() === MXGRAPH_ROOT_UUIDS[0]) {
				mxUtils.alert("The board layer cannot be removed!");
				return;
			}
	...
	var insertDiv = '<div class="geSprite geSprite-insert" style="display:inline-block;"></div>';
	var checkmarkDiv = '<div class="geSprite geSprite-checkmark" style="display:inline-block;"></div>';
	insertLink.innerHTML = insertDiv;
	...
	mxEvent.addListener(insertLink, 'click', function(evt)
	{
		if (graph.isEnabled() && !graph.isSelectionEmpty())
		{
			...
			insertLink.innerHTML = checkmarkDiv;
	...
	function renameLayer(layer)
	{
		if (graph.isEnabled() && layer != null)
		{
			...
			editorUi.showDialog(dlg.container, 350, 150, true, true);
			dlg.init();
		}
	};
	...
	var syncVisibilityLink = link.cloneNode();
	var syncVisibilityDiv = '<div class="geSprite geSprite-globe" style="display:inline-block;"></div>';
	syncVisibilityLink.innerHTML = syncVisibilityDiv;
	syncVisibilityLink.setAttribute('title', mxResources.get('syncLayer'));

	mxEvent.addListener(syncVisibilityLink, 'click', function(evt)
	{
		if (graph.isEnabled())
		{
			try {
				...
				{
					var layer = graph.model.getChildAt(graph.model.root, i);
					graph.model.execute(new mxVisibleChange(graph.model, layer, graph.model.isVisible(layer)));
				}

				syncVisibilityLink.innerHTML = checkmarkDiv;
	...
	if (!graph.isEnabled())
	{
		syncVisibilityLink.className = 'geButton mxDisabled';
	}

	ldiv.appendChild(syncVisibilityLink);
	...
	function refresh()
	{
		...
		function addLayer(index, label, child, defaultParent)
		{
			...
			{
				var willBeVisible = !graph.model.isVisible(child);
				graph.enableLazyTextPaint();
				...
				graph.disableLazyTextPaint();
				mxEvent.consume(evt);

				if(willBeVisible) {
					graph.showAll(child);
					graph.scrollCellsIntoView(graph.model.getChildren(child));
				}
			});
			...
			if (graph.getDefaultParent() == child)
			{
				...
				if(selectionLayer.getId() === MXGRAPH_ROOT_UUIDS[0] || !graph.isEnabled()) {
		...
		var label = graph.convertValueToString(selectionLayer) || 'Board';
	...
	this.window = new mxWindow(mxResources.get('layers'), div, x, y, w, h, false, true, undefined, 'layersWindow mxWindow');
	...
	this.window.setClosable(true);
	this.window.setVisible(true);
	this.window.closeImg.setAttribute('src', 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAkAAAAJAQMAAADaX5RTAAAABlBMVEV7mr3///+wksspAAAAAnRSTlP/AOW3MEoAAAAdSURBVAgdY9jXwCDDwNDRwHCwgeExmASygSL7GgB12QiqNHZZIwAAAABJRU5ErkJggg==')
	this.window.closeImg.style.marginRight = '5px';

	this.window.addListener(mxEvent.CLOSE, () => {
		graph.setDefaultParent(graph.model.getCell(MXGRAPH_ROOT_UUIDS[0]));
	});
