 ...
 */
EditorUi = function(editor, container, lightbox, boardName, sidebarContainerId, eventModelingEnabled)
{
	mxEventSource.call(this);
	this.destroyFunctions = [];
	this.editor = editor || new Editor(undefined, undefined, undefined, undefined, true, eventModelingEnabled);
	this.container = container || document.body;
	this.boardName = boardName;

	var graph = this.editor.graph;

	graph.setEditorUi(this);
	...
	this.createDivs(sidebarContainerId);
	this.createUi();
	this.refresh();
	...
	// Disable user selection to avoid that touch devices try to select nearby text
	// CellEditor enables it on-the-fly
		...
		else if (!mxEvent.isConsumed(evt) && evt.keyCode == 27 /* Escape */)
		{
			...
			if(!graph.isSelectionEmpty() && !graph.cellEditor.isContentEditing()) {
				if(graph.liteMode) {
					...
					if(cell && inspectioUtils.isContainer(cell) && !graph.hasAlternateStyle(cell)) {
						const action = this.actions.get('hide_container_details');
						action.funct();
						return;
					}
				}
	...
	graph.cellEditor.startEditing = function()
	{
		...
		if (graph.cellEditor.isContentEditing())
		{
			...
			var updateCssHandler = function()
			{
				if (!updating)
				{
					...
					window.setTimeout(function()
					{
						...
						if (node != null)
						{
							ui.toolbar.setFontStyle(inspectioUtils.isHeadingTag(node.tagName)? 'heading' : 'text');
							...
							if (css != null && ui.toolbar != null)
							{
								...
								const editingCell = graph.cellEditor.editingCell;
								const cellStyle = graph.getStylesheet().getCellStyle(graph.cellEditor.editingCell.getStyle());

								if(inspectioUtils.isIcon(editingCell)) {
									ui.toolbar.setCellBackgroundColor(inspectioUtils.getIconColor(cellStyle));
								} else {
									ui.toolbar.setCellBackgroundColor(cellStyle[mxConstants.STYLE_FILLCOLOR] || '#000000');
								}

								ui.toolbar.setCellType(inspectioUtils.getType(editingCell));
							}
			...
			const positionToolbar = () => {
				if (ui.toolbar != null && !ui.toolbar.isMovedByUser())
				{
					const textArea = graph.cellEditor.textarea;
					const textAreaHeight = Math.round(parseInt(textArea.offsetHeight) * graph.view.scale);

					let toolbarTop = parseInt(textArea.offsetTop) + textAreaHeight + 80;
					let toolbarLeft = parseInt(textArea.offsetLeft) - Math.round(parseInt(ui.toolbarContainer.offsetWidth) / 2);

					const toolbarHeight = parseInt(ui.toolbarContainer.offsetHeight);
					const toolbarWidth = parseInt(ui.toolbarContainer.offsetWidth);

					const mainContainerWidth = parseInt(ui.container.offsetWidth);
					const bottomBorder = parseInt(ui.bottomMenuContainer.offsetTop) - toolbarHeight - 10;
					const leftBorder = parseInt(ui.sidebarContainer.offsetWidth) + 20 /* resize handler / Border */;
					const rightBorder = parseInt(ui.container.offsetWidth) - 10;

					if(toolbarTop >= bottomBorder) {
						// Place toolbar above editing element
						toolbarTop = parseInt(textArea.offsetTop) - 60;
					}

					if(toolbarLeft <= leftBorder) {
						//Give up centered position and use padding from sidebar instead
						toolbarLeft = leftBorder + 10;
					}

					if((toolbarLeft + toolbarWidth) > rightBorder) {
						toolbarLeft = mainContainerWidth - 10 - toolbarWidth;
					}

					if(toolbarTop < parseInt(ui.sidebarContainer.offsetTop)) {
						toolbarTop = bottomBorder - 10 - parseInt(ui.toolbarContainer.offsetHeight);
					}

					ui.toolbarContainer.style.top = toolbarTop + "px";
					ui.toolbarContainer.style.left = toolbarLeft + "px";
				}
			};
			...
			if(graph.cellEditor.editingCell != null
				&& (inspectioUtils.isContainer(graph.cellEditor.editingCell)
				|| inspectioUtils.isTextField(graph.cellEditor.editingCell))) {
				mxEvent.addListener(graph.cellEditor.textarea, 'input', positionToolbar)
				mxEvent.addListener(graph.cellEditor.textarea, 'touchend', positionToolbar);
				mxEvent.addListener(graph.cellEditor.textarea, 'mouseup', positionToolbar);
			} else if (graph.cellEditor.editingCell != null && inspectioUtils.isSticky(graph.cellEditor.editingCell)) {
				mxEvent.addListener(graph.cellEditor.textarea, 'keyup', positionToolbar);
			}
		}
	};
	...
	if (this.toolbar != null)
	{
		...
		{
			var ff = graph.currentVertexStyle['fontFamily'] || 'Open Sans';
			var fs = String(graph.currentVertexStyle['fontSize'] || '14');
	...
	var graphStyle = graph.getGraphStylesheet();

	this.setBackgroundColor(graphStyle[ispConst.STYLE_GRAPH_BG_COLOR]);
	this.setGridColor(graphStyle[ispConst.STYLE_GRAPH_GRID_COLOR]);
	graph.view.gridSteps = graphStyle[ispConst.STYLE_GRAPH_GRID_STEPS];
	graph.view.minGridSize = graphStyle[ispConst.STYLE_GRAPH_GRID_MIN_GRID_SIZE];
	graph.setGridSize(graphStyle[ispConst.STYLE_GRAPH_GRID_SIZE]);
};
...
/**
 * Specifies the border of the sidebar
 */
EditorUi.prototype.splitSize = 30; /* (mxClient.IS_TOUCH || mxClient.IS_POINTER) ? 12 : 8; */
 ...
 * screen widths <= 640px.
 */
EditorUi.prototype.hsplitPosition = 55;
...
EditorUi.prototype.getBoardName = function()
{
	return this.boardName || 'prooph board';
}
...
EditorUi.prototype.onKeyDown = function(evt)
{
		...
		else
		{
			if(graph.isSelectionEmpty()) {
				...
				const tryNextCell = () => {
					...
					} else if (firstSelectedCell === selectedCell) {
						return;
					}

					if(!graph.isInViewPort(selectedCell)) {
						tryNextCell();
					} else {
						graph.scrollCellsIntoView([selectedCell]);
					}
				}
				...
				graph.selectCell(!mxEvent.isShiftDown(evt));
				var nextSelectedCell = graph.getSelectionCell();

				if(!graph.isInViewPort(nextSelectedCell)) {
					graph.scrollCellsIntoView([nextSelectedCell]);
				}
			}
		}
	...
	if(mxEvent.isControlDown(evt, true) && evt.which === 83) {
		mxEvent.consume(evt);
		this.actions.get('export').funct();
	}
};
...
EditorUi.prototype.initClipboard = function()
{
	...
	mxClipboard.copy = function(graph)
	{
		...
		{
			result = mxClipboardCopy.apply(this, arguments);
		}
...
EditorUi.prototype.initCanvas = function()
{
	...
	var lazyZoomDelay = this.lazyZoomDelay;
	var zoomTriggerCounter = 0;
	var zoomTriggerCounterReset = null;
	var scaleFactor = 0.06;
	var panningTimer = null;

	graph.lazyZoom = function(zoomIn, isTouchPad)
	{
		if(!this.isTouchpadModeEnabled) {
			isTouchPad = false;
		}
		...
		if(zoomTriggerCounterReset) {
			window.clearTimeout(zoomTriggerCounterReset);
		}

		zoomTriggerCounterReset = window.setTimeout(function () {
			zoomTriggerCounter = 0;
		}, 100);
		...
		if(zoomTriggerCounter > 5) {
			scaleFactor = isTouchPad? 0.012 : 0.03;
		}

		if(zoomTriggerCounter > 8) {
			scaleFactor = isTouchPad? 0.012 : 0.05;
		}

		if(zoomTriggerCounter > 10) {
			scaleFactor = isTouchPad? 0.012 : 0.07;
		}

		if(zoomTriggerCounter > 60) {
			scaleFactor = isTouchPad? 0.018 : 0.07;
		}
		...
		if (zoomIn)
		{
			if (this.view.scale * this.cumulativeZoomFactor < 0.1)
			{
				this.cumulativeZoomFactor = (this.view.scale + 0.01) / this.view.scale;
			}
			else
			{
				this.cumulativeZoomFactor = (this.view.scale + scaleFactor) / this.view.scale;
        ...
        this.updateZoomTimeout = window.setTimeout(mxUtils.bind(this, function()
        {
            ...
            this.zoom(this.cumulativeZoomFactor, undefined, isTouchPad);
	...
	var currentTranslateX = null;
	var currentTranslateY = null;

	var isDiagramOverlay = function (source) {
		if(!source || !source.closest) {
			return false;
		}

		return !!source.closest('.diagram-overlay');
	}
	...
	if (mxClient.IS_TOUCH)
	{
		...
		mxEvent.addListener(this.container, 'touchmove', mxUtils.bind(this, function(evt)
		{
			if(pinchStarted && mxEvent.isMultiTouchEvent(evt)) {
		...
		mxEvent.addListener(this.container, 'touchend', mxUtils.bind(this, function(evt)
		{
			if(pinchStarted) {
				pinchStarted = false;
				startDist = 0;
	...
	mxEvent.addMouseWheelListener(mxUtils.bind(this, function(evt, up)
	{
		...
		if ((this.dialogs == null || this.dialogs.length == 0) && graph.isZoomWheelEvent(evt))
		{
			...
			while (source != null)
			{
				if (source == graph.container || isDiagramOverlay(source))
				{
					...
					if((!isTouchPad && !graph.isTouchpadPanning)
						...
						|| (mxEvent.isTouchpadPinchToZoomEvent(evt) && !graph.isTouchpadPanning))
					{
						graph.lazyZoom(up, isTouchPad);
						mxEvent.consume(evt);
					} else if (isTouchPad || graph.isTouchpadPanning) {
						if(currentTranslateX === null && currentTranslateY === null) {
							currentTranslateX = graph.view.translate.x;
							currentTranslateY = graph.view.translate.y;
							graph.isTouchpadPanning = true;
						} else {

							if(graph.view.scale < 0.2) {
								currentTranslateX += (evt.deltaX * -1) / graph.view.scale;
								currentTranslateY += (evt.deltaY * -1) / graph.view.scale;
							} else {
								currentTranslateX += (evt.deltaX * -1 * 4);
								currentTranslateY += (evt.deltaY * -1 * 4);
							}
							...
							var me = new mxMouseEvent(evt, null);

							me.graphX = pt.x;
							me.graphY = pt.y;

							graph.fireEvent(new mxEventObject(
								graph.EVT_USER_IS_PANNING,
								'translate', graph.currentTranslate,
								'scale', graph.view.scale,
								'me', me,
								'dx', graph.currentTranslate.x,
								'dy', graph.currentTranslate.y
							));
						}

						if(panningTimer) {
							window.clearTimeout(panningTimer);
						}

						panningTimer = window.setTimeout(() => {
							currentTranslateX = null;
							currentTranslateY = null;
							graph.isTouchpadPanning = false;
						}, 200);
						mxEvent.consume(evt);
					}
...
EditorUi.prototype.refresh = function(sizeDidChange)
{
	...
	if(this.sidebarContainer.offsetHeight < 860) {
		effHsplitPosition = effHsplitPosition * 2;
	}
	 */
	...
	*/
	...
	// this.sidebarContainer.style.top = '45px';
	// this.sidebarContainer.style.width = effHsplitPosition + 'px';
	this.formatContainer.style.top = tmp + 'px';
	this.formatContainer.style.width = fw + 'px';
	this.formatContainer.style.display = (this.format != null) ? '' : 'none';

	this.diagramContainer.style.left = (this.hsplit.parentNode != null && false === true) ? (effHsplitPosition + this.splitSize) + 'px' : '0px';
	this.diagramContainer.style.top = '45px';
	this.footerContainer.style.height = this.footerHeight + 'px';
	// this.hsplit.style.top = this.sidebarContainer.style.top;
	// this.hsplit.style.bottom = '0px';/* (this.footerHeight + off) + 'px'; */
	...
	else
	{
		...
		// this.sidebarContainer.style.bottom = '0px'; /* (this.footerHeight + sidebarFooterHeight + off) + 'px'; */
		this.formatContainer.style.bottom = '0px'; /* (this.footerHeight + off) + 'px'; */
		this.diagramContainer.style.bottom = '0px'; /* (this.footerHeight + off + th) + 'px'; */
	}
 ...
 * Creates the required containers.
 */
EditorUi.prototype.createDivs = function(sidebarContainerId)
{
	...
	this.diagramContainer = this.createDiv('geDiagramContainer disable-user-select');
	this.footerContainer = this.createDiv('geFooterContainer');
	this.hsplit = this.createDiv('geHsplit');
	this.hsplit.setAttribute('title', mxResources.get('collapseExpand'));
	this.zoomMenuContainer = this.createDiv('inspectioZoomMenu');
	this.bottomMenuContainer = this.createDiv('inspectioBottomMenu');

	// Sets static style for containers
	this.zoomMenuContainer.style.bottom = '20px';
	this.zoomMenuContainer.style.right = '10px';
	this.bottomMenuContainer.style.left = '50%';
	this.bottomMenuContainer.style.bottom = '20px';
	this.bottomMenuContainer.style.marginRight = '-50%';
	this.menubarContainer.style.top = '0px';
	this.menubarContainer.style.left = '0px';
	this.menubarContainer.style.right = '0px';
	// this.sidebarContainer.style.left = '0px';
	this.toolbarContainer.classList.add('ui', 'menu');
 ...
 EditorUi.prototype.createUi = function()
{
	...
	if (this.toolbar != null)
	{
		...
		// @TODO replace with Semantic UI version
	...
	// Zoom Menu
	this.zoomMenu = (this.editor.chromeless) ? null : this.createZoomMenu(this.zoomMenuContainer);

	if(this.zoomMenu != null) {
		this.container.appendChild(this.zoomMenuContainer);
	}

	// Bottom Menu
	this.bottomMenu = (this.editor.chromeless) ? null : this.createBottomMenu(this.bottomMenuContainer);

	if(this.bottomMenu != null) {
		this.container.appendChild(this.bottomMenuContainer);
	}
};
...
EditorUi.prototype.createZoomMenu = function(container)
{
	return new ZoomMenu(this, container);
}

EditorUi.prototype.createBottomMenu = function(container)
{
	return new BottomMenu(this, container);
}
...
EditorUi.prototype.showIconDialog = function(fn, noStyle) {
	const dlg = new IconDialog(this, fn, noStyle);
	this.showDialog(dlg.container, 480, 370, true, true);
	dlg.init();
}
...
EditorUi.prototype.showLinkDialog = function(value, btnLabel, fn)
{
	...
	this.showDialog(dlg.container, 480, 160, true, true);
	dlg.init();
};
...
EditorUi.prototype.createKeyHandler = function(editor)
{
	...
	var count = 0;
	var factor = 1;
	var finalStepSize = null;
	var lastKeyCode = null;

	function resizeFunct (internalStepSize , keyCode) {
		if(finalStepSize === null) {
		...
		for (var i = 0; i < cells.length; i++)
		{
			if (graph.getModel().isVertex(cells[i]) && graph.isCellResizable(cells[i]))
			{
				...
				if (geo != null)
				{
					...
					if (keyCode == 37)
					{
						geo.width = Math.max(0, geo.width - internalStepSize);
					}
					else if (keyCode == 38)
					{
						geo.height = Math.max(0, geo.height - internalStepSize);
					}
					else if (keyCode == 39)
					{
						geo.width += internalStepSize;
					}
					else if (keyCode == 40)
					{
						geo.height += internalStepSize;
					}
	...
	function moveFunct (internalStepSize, keyCode) {
		if(finalStepSize === null) {
			finalStepSize = internalStepSize;
		} else {
			finalStepSize += internalStepSize;
	...
	function nudge(keyCode, stepSize, resize)
	{
		...
		const commit = (internalFinalStepSize, internalKeyCode, internalResize) => {
			count = 0;
			factor = 1;
			lastKeyCode = null;

			// First reset changes made without change notification
			graph.model.beginUpdateWithoutChangeNotifications();

			try {
				if(internalResize) {
					resizeFunct(internalFinalStepSize * -1, internalKeyCode);
				} else {
					moveFunct(internalFinalStepSize * -1, internalKeyCode);
				}
			} finally {
				graph.model.endUpdateWithoutChangeNotifications();
			}
			...
			try {
				if(internalResize) {
					resizeFunct(internalFinalStepSize, internalKeyCode);
				} else {
	...
	keyHandler.bindControlShiftKey(8, function() {graph.goToPreviousPosition(); }); // Ctrl+Shift+Backspace
	...
	if (!this.editor.chromeless || this.editor.editable)
	{
		...
		keyHandler.bindControlKey(77, () => { graph.showCellMetadata(true) }); // Ctrl+M
		keyHandler.bindAction(72, true, 'codywizard'); //Ctrl+H
		keyHandler.bindAction(8, false, 'delete'); // Backspace
		// keyHandler.bindAction(8, true, 'deleteAll'); // Backspace
		keyHandler.bindAction(46, false, 'delete'); // Delete
		keyHandler.bindAction(46, true, 'deleteAll'); // Ctrl+Delete
		keyHandler.bindAction(72, true, 'inserthorizontalrule', true); // Ctrl+Shift+H
		...
		keyHandler.bindAction(70, true, 'lookup_element'); // Ctrl+F
		...
		keyHandler.bindAction(86, true, 'pasteGraphCenter'); // Ctrl+V
		keyHandler.bindAction(71, true, 'trigger_cody');
		...
		keyHandler.bindAction(76, true, 'align_horizontal_left', true); // Ctrl+Shift+L
		keyHandler.bindAction(67, true, 'align_horizontal_center', true); // Ctrl+Shift+M
		keyHandler.bindAction(82, true, 'align_horizontal_right', true); // Ctrl+Shift+R
		keyHandler.bindAction(85, true, 'align_vertical_up', true); // Ctrl+Shift+U
		keyHandler.bindAction(77, true, 'align_vertical_middle', true); // Ctrl+Shift+M
		keyHandler.bindAction(68, true, 'align_vertical_down', true); // Ctrl+Shift+D
		keyHandler.bindAction(190, true, 'superscript'); // Ctrl+.
		keyHandler.bindAction(188, true, 'subscript'); // Ctrl+,
		keyHandler.bindKey(13, () => { if (graph.isEnabled()) {
			if(graph.liteMode) {
				const cell = graph.getSelectionCell();

				if(cell && inspectioUtils.isContainer(cell) && graph.hasAlternateStyle(cell)) {
					const action = this.actions.get('show_container_details');
					action.funct();
					return;
				}
			}
