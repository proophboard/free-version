...
Actions.prototype.init = function()
{
	...
	this.addAction('pasteGraphCenter', function(evt)
	{
		...
		if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent()))
		{
			...
			try
			{
				var cells = mxClipboard.paste(graph, undefined, undefined, true);
				...
				if (cells != null)
				{
					...
					if(cells.length > 0) {
						...
						if(state != null) {
							var tBounds = graph.translateBounds(state);
							bounds = new mxRectangle(dx + tBounds.x, dy + tBounds.y, tBounds.width,
								tBounds.height);
						} else {
							console.log("geometry", firstCell.getGeometry());
					...
					if(geo != null) {
						console.log(graph.container.clientWidth, " x ", graph.container.clientHeight)
