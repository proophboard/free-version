...
mxCellEditor.prototype.placeCaretAtEnd = function (el) {
    el.focus();
...
mxCellEditor.prototype.startEditing = function(cell, trigger)
{
    ...
    {
        this.textarea.className = 'mxCellEditor geContentEditable enable-user-select';
    }
    else
    {
        this.textarea.className = 'mxCellEditor mxPlainTextEditor enable-user-select';
    }
    ...
    this.graph.setMouseCursor('text');
}
...
mxCellEditor.prototype.stopEditing = function(cancel)
{
    ...
    try {
        ...
        if(inspectioUtils.isSliceLaneLabel(this.editingCell)) {
            const watermarks = this.graph.getConnectedWatermarks(this.editingCell.getId(), this.editingCell.parent);

            watermarks.forEach(w => {
                this.graph.labelChanged(w, value);
                this.graph.resizeCell(w, new mxRectangle(
                  w.getGeometry().x,
                  w.getGeometry().y,
                  this.currentWidth,
                  this.currentHeight
    ...
    if(!cancel) {
        this.graph.disableUserSelectOnContainer();
    }
...
mxCellEditor.prototype.applyValue = function(state, value)
{
    ...
    try
    {
        ...
        if(inspectioUtils.isSliceLaneLabel(state.cell)) {
            const watermarks = this.graph.getConnectedWatermarks(state.cell.getId(), state.cell.parent);
    ...
    if(state.cell.isVertex()) {
        ...
        if(inspectioUtils.isContainer(state.cell)) {
            ...
            existingDeployTags = existingTags.filter(t => deployTags.includes(t));
            existingTags = existingTags.filter(t => !deployTags.includes(t));
        }

        const connectedTags = [
            ispConst.TAG_CONNECTED, ispConst.TAG_DISCONNECTED, ispConst.TAG_LANE_HANDLE, ispConst.TAG_TIME_HANDLE, ispConst.TAG_LANE_WATERMARK,
